...
function verifySignature(provider, rawBody, headers, secret) {
  try {
    ...
    if (provider === 'clerk') {
      ...
      // Reject replays: svix-timestamp must be within ±tolerance of server clock.
      // Tolerance is configurable via FANHOOK_CLERK_TOLERANCE_SECONDS (default 300).
      const toleranceRaw = parseInt(process.env.FANHOOK_CLERK_TOLERANCE_SECONDS, 10);
      const toleranceSec = Number.isFinite(toleranceRaw) && toleranceRaw >= 0 ? toleranceRaw : 300;
      const tsNum = parseInt(String(ts), 10);
      if (!Number.isFinite(tsNum)) {
        return { valid: false, error: 'svix-timestamp is not a valid integer' };
      }
      const nowSec = Math.floor(Date.now() / 1000);
      if (Math.abs(nowSec - tsNum) > toleranceSec) {
        return { valid: false, error: 'svix-timestamp outside tolerance window (replay protection)' };
      }
