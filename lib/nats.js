...
async function _doConnect() {
  ...
  // Reset the singleton when this connection closes so that subsequent
  // getConnection() calls can attempt a fresh reconnect (e.g. after NATS
  // restarts while an SSE stream is holding a backoff-retry loop).
  (async () => {
    await conn.closed();
    if (nc === conn) {
      nc = null;
      ...
      console.log('[nats] Connection closed — singleton reset for reconnect');
    }
  })();
...
/**
 * Publish a real-time status event to NATS core (not JetStream — no persistence needed).
 * Used by ingest and the delivery worker to push live updates to SSE subscribers.
 * Fire-and-forget — callers should swallow errors.
 *
 * @param {string} sinkId
 * @param {{ event_id: string, sink_id: string, status: string, received_at?: string }} data
 */
async function publishStatusEvent(sinkId, data) {
  const conn = await getConnection();
  conn.publish(`events.status.${sinkId}`, jc.encode(data));
}
...
async function listDLQMessages(sinkId, limit = 50) {
  ...
  // Create an ephemeral durable pull consumer scoped to this sink's DLQ subject.
  // We previously used the OrderedConsumer API (`js.consumers.get(stream, opts)`),
  // but in nats.js 2.29 + nats-server 2.10 OrderedConsumer.fetch() never returns
  // — both `fetch` and `consume` hang indefinitely. A raw durable pull consumer
  // with AckPolicy.None behaves identically for read-only listing and works
  // reliably. The DLQ stream bounds each subject to 500 messages
  // (LimitsPolicy max_msgs_per_subject), so this fetch is always O(500).
  const name = `dlq_list_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  try {
    await jsm.consumers.add(DLQ_STREAM_NAME, {
      durable_name: name,
  ...
  const msgs = [];
  try {
    const consumer = await js.consumers.get(DLQ_STREAM_NAME, name);
    ...
    try { await jsm.consumers.delete(DLQ_STREAM_NAME, name); } catch (_) {}
  }
