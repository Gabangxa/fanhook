...
async function listDLQMessages(sinkId, limit = 50) {
  ...
  const jsm = await nc.jetstreamManager();
  const subject = `${DLQ_SUBJECT_PREFIX}.${sinkId}`;
  const tempName = `dlq-reader-${sinkId}-${Date.now()}`;

  try {
    await jsm.consumers.add(DLQ_STREAM_NAME, {
      durable_name: tempName,
      ack_policy: AckPolicy.Explicit,
      deliver_policy: DeliverPolicy.All,
      filter_subject: subject,
    });

    const consumer = await js.consumers.get(DLQ_STREAM_NAME, tempName);
    const msgs = [];
    ...
    try {
      ...
      // Fetch timeout is expected when fewer messages exist than limit
    }

    // Return most-recent-first (fetch returns oldest-first)
    return msgs.reverse();
  } finally {
    try { await jsm.consumers.delete(DLQ_STREAM_NAME, tempName); } catch (_) {}
  }
}
