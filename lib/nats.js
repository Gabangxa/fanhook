...
async function listDLQMessages(sinkId, limit = 50) {
  ...
  // Get stream state to calculate a smart start sequence.
  // We use a window of limit*10 from the end so that, even in a multi-tenant stream
  // where messages from different sinks are interleaved, we scan enough recent
  // messages to reliably surface the last `limit` entries for this sink.
  let startSeq;
  try {
    const info = await jsm.streams.info(DLQ_STREAM_NAME);
    const s = info.state;
    if (s.messages === 0) return [];
    startSeq = Math.max(s.first_seq, s.last_seq - limit * 10 + 1);
  } catch (_) {
    return [];
  }

  // Create an ephemeral consumer (no durable_name = ordered consumer semantics).
  // Starting at opt_start_seq ensures we read from the tail of the stream.
  let ephemeralName;
  try {
    const consumerInfo = await jsm.consumers.add(DLQ_STREAM_NAME, {
      // Intentionally no durable_name → ephemeral/ordered consumer
      ack_policy: AckPolicy.Explicit,
      deliver_policy: DeliverPolicy.ByStartSequence,
      opt_start_seq: startSeq,
      filter_subject: subject,
    });
    ephemeralName = consumerInfo.name; // Server assigns a name to ephemeral consumers
  } catch (_) {
    return [];
  }
  ...
  try {
    ...
    try {
      // Fetch up to limit*10 messages (generous window for multi-tenant streams).
      // Not acking — LimitsPolicy stream retains messages regardless; the consumer
      // is discarded immediately after use.
      const iter = await consumer.fetch({ max_messages: limit * 10, expires: 1500 });
      ...
      // Timeout is expected when the stream has fewer messages than the fetch cap
    }
