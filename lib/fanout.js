...
// Agents whose DNS lookup enforces the SSRF policy at socket-connect time
// (defeats DNS rebinding: the address actually connected to is the one that
// was validated). Policy is read from env on every lookup call.
const safeLookup = ssrf.makeSafeLookup();
const safeHttpAgent = new http.Agent({ lookup: safeLookup });
const safeHttpsAgent = new https.Agent({ lookup: safeLookup });
const pickAgent = (parsedUrl) =>
  parsedUrl.protocol === 'https:' ? safeHttpsAgent : safeHttpAgent;

// Explicit per-request timeout so a hanging destination cannot pin a delivery
// slot indefinitely. Override with FANHOOK_FANOUT_TIMEOUT_MS.
const FANOUT_TIMEOUT_MS = (() => {
  const n = parseInt(process.env.FANHOOK_FANOUT_TIMEOUT_MS || '', 10);
  return Number.isFinite(n) && n > 0 ? n : 10_000;
})();

// Default attempt delays: 1: immediate, 2: 30s, 3: 120s.
// Tests override via FANHOOK_RETRY_DELAYS_MS (comma-separated ms values) so the
// suite finishes in seconds instead of minutes. Example: "0,50,50".
const RETRY_DELAYS_MS = (() => {
  const raw = process.env.FANHOOK_RETRY_DELAYS_MS;
  if (!raw) return [0, 30_000, 120_000];
  const parsed = raw.split(',').map((s) => Number(s.trim())).filter((n) => Number.isFinite(n));
  return parsed.length === 3 ? parsed : [0, 30_000, 120_000];
})();
...
async function deliverToRoute(db, eventId, route, rawBody, headers) {
  ...
  // SSRF re-validation immediately before delivery. Routes are validated at
  // creation time too, but a DNS-rebinding attack can change what a hostname
  // resolves to between creation and delivery — so re-check every time.
  // A blocked destination is a permanent failure: record one failed attempt
  // and skip the retry schedule entirely.
  try {
    await ssrf.assertPublicDestination(route.url);
  } catch (err) {
    const message = `Blocked destination: ${err.message}`;
    db.prepare(`
      INSERT INTO delivery_attempts (id, event_id, route_id, attempt_number, status, http_status, attempted_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(uuidv4(), eventId, route.id, 1, 'failed', null, new Date().toISOString());
    return { success: false, http_status: null, attempt_number: 1, error_message: message };
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    const timeoutTimer = setTimeout(() => controller.abort(), FANOUT_TIMEOUT_MS);
    try {
      ...
      // Network error or timeout — treat as failed attempt
      httpStatus = null;
      success = false;
      errorMessage = (err && err.name === 'AbortError')
        ? `Timeout after ${FANOUT_TIMEOUT_MS}ms`
