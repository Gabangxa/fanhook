...
function verifyCsrf(req) {
  ...
  // Accept either a form-style `_csrf` body field OR an `X-CSRF-Token` header
  // (used by JSON `fetch()` calls from the dashboard).
  const headerToken = req.headers && req.headers['x-csrf-token'];
  const bodyToken = (req.body && req.body._csrf) || headerToken;
  if (
